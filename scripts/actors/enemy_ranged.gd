...
@export var hit_flash_color: Color = Color(0.92, 1.0, 1.0, 1.0)
...
func _draw() -> void:
	...
	draw_circle(Vector2.ZERO, marker_radius + 6.0, Color(1.0, 0.86, 0.25, 0.12))
	draw_arc(Vector2.ZERO, marker_radius, 0.0, TAU, 56, Color(1.0, 0.95, 0.45, 0.95), 4.0, true)
...
func _physics_process(delta: float) -> void:
	...
	var to_target: Vector2 = movement_target.global_position - global_position
	var desired_velocity: Vector2 = _compute_ranged_velocity(to_target, delta)
	velocity = desired_velocity
	move_and_slide()

	_update_visual_orientation(to_target)
	_apply_contact_damage()
	_tick_shooting(delta, to_target)
	_update_hit_flash(delta)
	_update_animation_state(velocity.length() > 0.01)

func _compute_ranged_velocity(to_target: Vector2, delta: float) -> Vector2:
	...
	var to_target_dir: Vector2 = to_target / distance_value
	if distance_value > preferred_range:
		return to_target_dir * move_speed
	if distance_value < retreat_range:
		return -to_target_dir * move_speed

	_strafe_switch_timer = maxf(0.0, _strafe_switch_timer - delta)
	if _strafe_switch_timer <= 0.0:
		_strafe_sign *= -1.0
		_strafe_switch_timer = randf_range(0.75, 1.75)
	var strafe_dir: Vector2 = Vector2(-to_target_dir.y, to_target_dir.x) * _strafe_sign
	return strafe_dir * (move_speed * 0.78)

func _tick_shooting(delta: float, to_target: Vector2) -> void:
	...
	if distance_to_target > shot_range:
		_shot_timer = 0.2
		return

	_fire_projectile(to_target.normalized())
	_shot_timer = maxf(0.22, shot_interval_seconds)
...
func _fire_projectile(direction: Vector2) -> void:
	...
	projectile.global_position = global_position + direction * projectile_spawn_distance
...
func apply_spawn_scaling(speed_multiplier: float, hp_multiplier: float, damage_multiplier: float) -> void:
	...
	shot_interval_seconds = maxf(0.20, shot_interval_seconds / safe_speed_multiplier)
...
func apply_elite_profile(
	...
	move_speed = maxf(1.0, move_speed * safe_speed_multiplier)
	shot_interval_seconds = maxf(0.18, shot_interval_seconds / safe_speed_multiplier)
	max_hp = maxi(1, int(round(float(max_hp) * safe_hp_multiplier)))
	current_hp = max_hp
	contact_damage = maxi(1, int(round(float(contact_damage) * safe_damage_multiplier)))
	shot_damage = maxi(1, int(round(float(shot_damage) * safe_damage_multiplier)))
...
func _setup_animated_sprite() -> void:
	if animated_sprite == null:
		push_error("EnemyRanged requires an AnimatedSprite2D child node named AnimatedSprite2D.")
		return
	...
	if animated_sprite.sprite_frames == null:
		push_error("EnemyRanged AnimatedSprite2D requires authored SpriteFrames (no auto generation).")
		return
...
func _update_visual_orientation(to_target: Vector2) -> void:
	if animated_sprite == null:
		return
